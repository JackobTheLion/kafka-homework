...
@Slf4j
public class PullConsumer extends AbstractConsumer {

    private static final String NAME = "Pull consumer";

    public PullConsumer(List<String> topics) {
    ...
    private Properties getProperties() {
        ...
        properties.put(ConsumerConfig.GROUP_ID_CONFIG, "pull_group"); //Консьюмер группа. У каждого консьюмера своя, чтобы вычитывать все сообщения
        properties.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest"); //Начать вычитку с самого раннего невычитанного офсета
        properties.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "false"); //Выключаем автокомит
